...
Блок из двух **taks**, одна из которых (`rescue`) выполняется при провале выполнения первой.
...
Установка загруженных пакетов через пакетный менеджер `yum`.
...
Конфигурирования **Clickhouse** путём копирования (модуль `ansible.builtin.copy`) подготовленных файлов на целевую машину.
